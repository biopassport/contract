...
/**
 * BiotCoin contract learning test
 *   deploy in before, and no deploy between it()
 */
contract("BiotCoin", (accounts) => {
  ...
  let NoOfTokensInMinunit, OneBiotCoinInMinunit, NoOfTokensInBiot;
  let coin
  ...
  before(async () => {
    ...
    OneBiotCoinInMinunit = BIG(await coin.getOneBiotCoin());
    NoOfTokensInBiot = NoOfTokensInMinunit.div(OneBiotCoinInMinunit)
  });
  ...
  it("coin initial values", async () => {
    ...
    assert.equal(await coin.symbol(), "BIOT");
    assert.equal(await coin.name(), "BIOTCoin");
    assert.equal(await coin.decimals(), 9);
    assert.equal((await coin.totalSupply()).toString(), BIG(NOOFTOKENS).mul(OneBiotCoinInMinunit).toString());
  ...
  it("token distribution", async () => {
    ...
    await coin.transfer(admin, OneBiotCoinInMinunit, {from: vault});
    assert.equal(await balanceOf(admin), OneBiotCoinInMinunit.toString());

    // transfer 0.5 token from admin to owner
    let halfBiot = OneBiotCoinInMinunit.div(BIG(2))
    await coin.transfer(owner, halfBiot, {from: admin});
    assert.equal(await balanceOf(owner), halfBiot.toString());
    assert.equal(await balanceOf(admin), halfBiot.toString());
    assert.equal(await balanceOf(vault), (NoOfTokensInBiot.sub(BIG(1)).mul(OneBiotCoinInMinunit).toString()));
  });
  ...
  it("marginal transfers", async () => {
    ...
    try {
      await coin.transfer(user2, OneBiotCoinInMinunit.mul(BIG(1)), {from: user1});
      assert.fail();
    }
    ...
    try
    {
      await coin.transfer(user2, OneBiotCoinInMinunit.mul(BIG(0)), {from: user1});
      assert.equal(await balanceOf(user1), OneBiotCoinInMinunit.mul(BIG(0)).toString());
      assert.equal(await balanceOf(user2), OneBiotCoinInMinunit.mul(BIG(0)).toString());
    }
    catch(exception) { assert.fail(); }

    // transfer 0 token from vault(balance > 0) to user1
    try {
        await coin.transfer(user1, OneBiotCoinInMinunit.mul(BIG(0)), {from: vault});
        assert.equal(await balanceOf(user1), OneBiotCoinInMinunit.mul(BIG(0)).toString());
    } catch(exception) { assert.fail(); }

    // transfer whole balance to other
    await coin.transfer(user1, OneBiotCoinInMinunit.mul(BIG(1)), {from: vault});
    assert.equal(await balanceOf(user1), OneBiotCoinInMinunit.mul(BIG(1)).toString());
    await coin.transfer(user2, OneBiotCoinInMinunit.mul(BIG(1)), {from: user1});
    assert.equal(await balanceOf(user1), OneBiotCoinInMinunit.mul(BIG(0)).toString());
    assert.equal(await balanceOf(user2), OneBiotCoinInMinunit.mul(BIG(1)).toString());
  });
  ...
  it("only admin can set reserve", async () => {
    await coin.setReserve(user1, OneBiotCoinInMinunit, {from: admin});
  ...
  it("reserve and reserveOf", async () => {
    ...
    await coin.transfer(user1, OneBiotCoinInMinunit.mul(BIG(10)), { from: vault });
    await coin.setReserve(user1, OneBiotCoinInMinunit.mul(BIG(5)), { from: admin });
    assert.equal(await reserveOf(user1), OneBiotCoinInMinunit.mul(BIG(5)).toString());
  });
  
  it("transfer after setting reservation", async () => {
    // user1 = 10 Biot
    await coin.transfer(user1, OneBiotCoinInMinunit.mul(BIG(10)), { from: vault });
    // user1 = 10 Biot with 5 Biot reserve
    await coin.setReserve(user1, OneBiotCoinInMinunit.mul(BIG(5)), { from: admin });
    // user1 = 9 Biot with 5 Biot reserve
    await coin.transfer(user2, OneBiotCoinInMinunit.mul(BIG(1)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneBiotCoinInMinunit.mul(BIG(9)).toString());

    // user1 = 6 Biot with 5 Biot reserve
    coin.transfer(user2, OneBiotCoinInMinunit.mul(BIG(3)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneBiotCoinInMinunit.mul(BIG(6)));

    try {
      let tx = await coin.transfer(user2, OneBiotCoinInMinunit.mul(BIG(2)), {from: user1}); // violating reserved amount
      assert.equal(await balanceOf(user1), OneBiotCoinInMinunit.mul(BIG(6)));
      assert.fail();
    }
    catch(exception) {
